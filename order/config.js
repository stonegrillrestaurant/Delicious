...
// ====== MENU (sample; keep your latest prices later) ======
const menuItems = {
  pork: [
...
let paymentDialogAllowed = false; // safety so QR never shows on load
...
// Resolve first existing image from candidate list
function resolveAndSetImg(elId, candidates) {
  ...
  const tryNext = (i) => {
    ...
    probe.onload = () => { target.src = candidates[i]; };
...
`;
  ...
  const tPayload = { chat_id: CFG.-1002531095369, text: msg, parse_mode: "Markdown" };
...
// ====== EVENT BINDINGS ======
function initEvents() {
  // Force-hide all popups on load (in case of old cache)
...
function boot() {
  ...
  // Resolve assets against whatever filenames you already have
  resolveAndSetImg("logoImg", CFG.ASSETS.logo);
  resolveAndSetImg("gcashQR", CFG.ASSETS.gcashQR);
}
